...
void main() {
  ...
  group('backgroundColor / background / foreground / shadows (flattener)', () {
    ...
    test('backgroundColor flows into runs and emits rects', () {
      final items = flatten(const TextStyle(
        fontFamily: 'Lato',
        fontSize: 16,
      ...
      final para = wf.breakLines(
          items, double.infinity, const wf.ParagraphStyle());
      final emitted =
          wf.emitInstances(para, 400, wf.TextAlign.left, null);
      expect(emitted.backgrounds, isNotEmpty);
      final line = para.lines.single;
      final total =
          emitted.backgrounds.fold<double>(0, (w, b) => w + b.width);
    ...
    test('shadows flow into runs and emit shadow boxes', () {
      final items = flatten(const TextStyle(
        ...
        fontSize: 16,
        shadows: [
          Shadow(offset: Offset(2, 3), blurRadius: 4, color: Color(0xFF112233)),
      ...
      final total =
          emitted.shadowRuns.fold<double>(0, (w, b) => w + b.width);
  ...
  group('strut (VM semantics)', () {
    ...
    test('acts as a floor on line metrics', () {
      final big = wf.breakLines([run('hello')], double.infinity,
      ...
      final small = wf.breakLines([run('hello')], double.infinity,
          const wf.ParagraphStyle(
              strut: wf.StrutMetrics(ascent: 5, descent: 2)));
    ...
    test('leading splits half above, half below', () {
      final para = wf.breakLines([run('hello')], double.infinity,
          const wf.ParagraphStyle(
    ...
    test('force replaces text metrics', () {
      final para = wf.breakLines([run('hello')], double.infinity,
          const wf.ParagraphStyle(
              strut:
                  wf.StrutMetrics(ascent: 10, descent: 5, force: true)));
    ...
    test('blank lines are strutted too', () {
      final para = wf.breakLines([run('a\n\nb')], double.infinity,
          const wf.ParagraphStyle(
              strut: wf.StrutMetrics(ascent: 40, descent: 10)));
  ...
  group('text height behavior (VM semantics)', () {
    ...
    test('applyHeightToFirstAscent=false reverts the first ascent only', () {
      ...
      final trimmed = wf.breakLines([run('a\nb')], double.infinity,
    ...
    test('even leading splits the height excess symmetrically', () {
      ...
      final even = wf.breakLines([run('a')], double.infinity,
      ...
      final overridden = wf.breakLines([run('a', evenLeading: true)],
  ...
  group('widget plumbing', () {
    ...
    testWidgets('overflow fade and backgrounds paint cleanly',
        (tester) async {
