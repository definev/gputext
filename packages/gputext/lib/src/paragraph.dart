...
ParagraphLines _layoutPreparedLines(
  ...
  if (lines.isEmpty) {
    ...
    if (styleItem >= 0 && prepared.items[styleItem] is TextRun) {
      _growMetrics(line, prepared.items[styleItem] as TextRun, style);
    }
...
LineMetrics _materializeLine(
  ...
  final lastSeg = range.endGrapheme > 0
      ? range.endSegment
      : range.endSegment - 1;

  for (
    var seg = range.startSegment;
    ...
    if (pieces.first.isAtomic) {
      ...
      if (item is EmojiItem) {
        ...
        line.items.add(
          LinePlaceholder(item, itemIndex: pieces.first.itemIndex),
        );
    ...
    while (g < gLast) {
      ...
      line.items.add(
        LineRun.fromRun(
          run,
          segText.substring(startOff, offs[end - 1]),
          w,
          itemIndex: piece.itemIndex,
    ...
    for (final item in line.items) {
      ...
      if (deco != null && deco.isActive && pen > penStart) {
        ...
        if (deco.underline) {
          ...
          addLine(
            baselineY - m.underlinePosition * scale + underTh / 2,
            underTh,
            false,
          );
        }
        ...
        if (deco.lineThrough) {
          addLine(
            baselineY - m.strikeoutPosition * scale,
            m.strikeoutSize * scale,
            true,
          );
        }
      }
