...
PreparedParagraph _prepareParagraph(List<InlineItem> items) {
    ...
    if (allowBreaks) {
      ...
      for (final p in rawPieces) {
        ...
        for (var g = 0; g < cum.length; g++) {
          final a = (cum[g] - prev) * scale + run.letterSpacingPx * runes[g];
          prev = cum[g];
          adv.add(a);
          pieceWidth += a;
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      switch (kind) {
        case SegmentBreakKind.hardBreak:
          ...
          final styleItem = owner.isEmpty
              ? lastRunIndex
              : owner.first.itemIndex;
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    pushSegment(SegmentBreakKind.text, width, '', [
      SegmentPiece.atomic(i, width),
    ]);
  }
  flushWindow();
  ...
  if (chunkStart < widths.length) {
    chunks.add(
      LineBreakChunk(chunkStart, widths.length, widths.length, lastRunIndex),
    );
    if (chunkSum > maxIntrinsic) maxIntrinsic = chunkSum;
  }
