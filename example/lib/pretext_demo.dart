...
TextSpan _showcaseSpan() => const TextSpan(
  ...
  children: [
    ...
    TextSpan(
      text:
          'soft\u00ADhy\u00ADphen\u00ADat\u00ADed words show a hyphen '
...
final _features = <_FeatureRow>[
  ...
  _FeatureRow(
    ...
    const TextSpan(
      style: _featureStyle,
      text:
          'Long compounds like Donau\u200Bdampf\u200Bschiff\u200Bfahrt '
          'wrap at seams.',
    ),
    130,
  ),
...
class _PretextDemoPageState extends State<PretextDemoPage> {
  ...
  void initState() {
    ...
    final scrollPreset = double.tryParse(
      Platform.environment['GPUTEXT_DEMO_SCROLL'] ?? '',
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 12,
            children: [
              _fixedPane('gputext', f.width, GPURichText(text: f.span)),
  ...
  Widget _fixedPane(String label, double width, Widget child) => Column(
    ...
    children: [
      Text(
        label,
        style: TextStyle(fontSize: 10, color: _ink.withValues(alpha: 0.45)),
      ),
      ...
      child: Padding(
        ...
        child: DefaultTextStyle(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'prepare / layout split · 1000-word paragraph',
                style: TextStyle(fontWeight: FontWeight.w700),
              ),
              const SizedBox(height: 6),
              Text(
                'first prepare (cold, fills the segment-metrics cache): '
                '${_prepareUs.toStringAsFixed(0)} µs',
              ),
              Text(
                'layout at ${width.round()}px '
                '(${lines.lines.length} lines): '
                '${layoutUs.toStringAsFixed(1)} µs',
              ),
              Text(
                'one-shot re-prepare + layout (pre-port shape): '
