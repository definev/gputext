...
class AtlasTextureUploader {
  ...
  AtlasTextures upload(
      gpu.GpuContext context, List<double> curves, List<int> rows) {
    ...
    final needCurveH =
        ((curveVec2Count + curveTexWidth - 1) ~/ curveTexWidth).clamp(1, maxH);
    ...
    final needRowH =
        ((rowCount * 2 + curveTexWidth - 1) ~/ curveTexWidth).clamp(1, maxH);
