...
double _monoRootT(
    double a, double b, double e0, double e1, double v, bool rising) {
...
double _bandWindingArea(
    List<double> pieces, List<int> bucket, double x0, double b0, double b1) {
  var area = 0.0;
...
BandHeader bandPieces(
  ...
  for (var k = 0; k < n; k++) {
    final yLo = math.min(
        pieces[k * 6 + 1], math.min(pieces[k * 6 + 3], pieces[k * 6 + 5]));
    final yHi = math.max(
        pieces[k * 6 + 1], math.max(pieces[k * 6 + 3], pieces[k * 6 + 5]));
    ...
    xLeft = math.min(
        xLeft,
        math.min(pieces[k * 6],
            math.min(pieces[k * 6 + 2], pieces[k * 6 + 4])));
  }

  final rowBase = rowOut.length ~/ 5;
  final bandH = r > 1 ? (y1 - y0) / r : y1 - y0;
  double xMax(int k) => math.max(
      pieces[k * 6], math.max(pieces[k * 6 + 2], pieces[k * 6 + 4]));
  double xMin(int k) => math.min(
      pieces[k * 6], math.min(pieces[k * 6 + 2], pieces[k * 6 + 4]));
  ...
  for (var b = 0; b < r; b++) {
    ...
    final area = n != 0
        ? _bandWindingArea(
            pieces, bucket, xLeft, y0 + b * bandH, y0 + (b + 1) * bandH)
        : 0.0;
    rowOut.addAll(
        [start, bucket.length, f32bits(area), f32bits(bxMin), f32bits(bxMax)]);
  }
