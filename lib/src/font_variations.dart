...
double _tupleScalar(
  ...
  for (var a = 0; a < peaks.length; a++) {
    ...
    if (starts != null && ends != null) {
      ...
      scalar *= v < peak ? (v - start) / (peak - start) : (end - v) / (end - peak);
    } else {
...
class _Mvar {
  ...
  static _Mvar parse(ByteData d, int off) {
    ...
    for (var i = 0; i < valueRecordCount; i++) {
      records[_tag4(d, p)] =
          (d.getUint16(p + 4, Endian.big), d.getUint16(p + 6, Endian.big));
      p += valueRecordSize;
    }
    return _Mvar(
...
class _Gvar {
  _Gvar(this._data, this._axisCount, this._sharedTuples, this._dataStart,
      this._offsets);
