...
TextSegments _classify(String text) {
  ...
  for (final cp in text.runes) {
    ...
    final symbolContinues = kind == SegmentBreakKind.text &&
        !wordLike &&
        cp == runLastCp &&
        cp <= 0xFFFF &&
        cp != 0x2D &&
    ...
    final neverGroups = kind == SegmentBreakKind.hardBreak ||
  ...
  for (var i = s.length - 1; i >= 0; i--) {
    ...
    if (kinds[i] == SegmentBreakKind.text &&
        ...
        (_isForwardStickyClusterSegment(texts[i]) ||
            (texts[i] == '-' && _startsWithDecimalDigit(texts[nextLiveIndex])))) {
...
TextSegments _mergeUrlRuns(TextSegments s) {
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    final queryStart =
        queryStartOverride < 0 ? s.starts[nextIndex] : queryStartOverride;
    var j = nextIndex;
    while (j < s.length && !_isTextRunBoundary(s.kinds[j])) {
    ...
    if (kind == SegmentBreakKind.text) {
      ...
      while (j < s.length &&
          ...
          _canJoinNoSpaceWordBoundary(
              prevText, prevWordLike, s.texts[j], s.isWordLike[j])) {
        parts.write(s.texts[j]);
