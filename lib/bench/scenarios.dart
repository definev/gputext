...
import '../src/paragraph.dart' as wf show KnuthPlassLineBreaker, LineBreaker;
...
class BenchContext {
  BenchContext({required this.corpus, required this.quick, required this.hasCjk});
...
List<FrameScenario> frameScenarios(BenchContext ctx) {
  ...
  return [
    ...
    FrameScenario(
      ...
      desc: 'RenderComparison.layout each frame: full flatten+prepare+break '
        ...
        child: ListView.builder(
          ...
          itemBuilder: (_, i) => Padding(
            ...
            child: benchText(
                e,
                TextSpan(
                    text: ctx.corpus.gatsbyParagraphs[i], style: benchStyle())),
          ),
        ),
      ),
    ),
    FrameScenario(
      ...
      build: (ctx, e, tick) {
        final scale =
            math.pow(8, _triangle(tick / ctx.measureFrames)).toDouble();
        return SizedBox(
          width: 420,
          height: 620,
          child: ClipRect(
    ...
    FrameScenario(
      ...
      desc: 'Many-widget overhead: 240 small paragraphs all visible; '
    ...
    FrameScenario(
      ...
      desc: 'reflow_width driver with TextAlign.justify (greedy breaks both '
          'sides).',
      path: 'pure',
      build: (ctx, e, tick) {
        final width = 330 + 90 * math.sin(tick * 2 * math.pi / 120);
    ...
    FrameScenario(
      ...
      build: (ctx, e, tick) {
        ...
        return _column([
          for (var i = 0; i < 12; i++)
            benchText(e, TextSpan(text: sentences[i], style: benchStyle()),
                align: TextAlign.justify, knuthPlass: true),
        ], width: width);
      },
    ),
    FrameScenario(
      ...
      desc: 'GoogleSansFlex wght 100↔1000 in 64 quantized steps: each new '
    ...
    FrameScenario(
      ...
      desc: 'Hybrid path: emoji clusters and uncovered characters expand '
        ...
        return _column([
          for (var i = 0; i < math.min(12, lines.length); i++)
            benchText(
                e,
                TextSpan(
                    text: '${lines[i]} #${tick % 100}', style: benchStyle())),
        ]);
      },
    ),
    FrameScenario(
      ...
      build: (ctx, e, tick) => _column([
        for (var i = 0; i < 12; i++)
          benchText(
              e,
              TextSpan(style: benchStyle(), children: [
                for (var w = 0; w < 8; w++) ...[
                  TextSpan(
                      text: ' ${sentences[i].split(' ').skip(w * 3).take(3).join(' ')}'
                          '${w == 0 ? ' #$tick' : ''}'),
                  const WidgetSpan(
