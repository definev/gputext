...
import '../src/text/metrics_cache.dart' show debugClearSegmentMetricsFor;
import '../src/widgets/span_flattener.dart';
import 'corpus.dart';
import 'report.dart';
...
class CpuTier {
  ...
  TextSpan _span(String text, {String family = 'Lato'}) => TextSpan(
        text: text,
        style: TextStyle(
            fontFamily: family, fontSize: _fontSize, color: const Color(0xFF000000)),
      );

  List<wf.InlineItem> _flatten(String text, {String family = 'Lato'}) {
    final items =
        flattenSpan(_span(text, family: family), TextScaler.noScaling, engine);
    if (items == null) {
  ...
  Future<void> run(BenchReport report, void Function(String) progress) async {
    ...
    final coldRt = await bench((_) {
      for (final t in texts) {
        final p = TextPainter(
            text: _span(t), textDirection: TextDirection.ltr)
          ..layout(maxWidth: 400);
        sink += p.height;
        p.dispose();
      }
    ...
    final warmRt = await bench((r) {
      final width = _widths[r % _widths.length];
      for (final p in painters) {
        p.layout(maxWidth: width);
        sink += p.height;
      }
    }, sampleRepeats: _widths.length, runs: _runs);
    for (final p in painters) {
      p.dispose();
    }
    report.cpuResults.add(cpuResult(
      ...
      desc: 'Relayout at cycled widths on retained painters — the engine has '
    ...
    final corpora = <(String, String, String, GPUFont?)>[
      ...
      ('synthetic-long-breakable', buildLongBreakableStressText(quick ? 40 : 220),
          'Lato', lato),
    ];
    for (final (id, text, family, font) in corpora) {
      ...
      final coldPrep = await bench((_) {
        debugClearSegmentMetricsFor(font);
        sink += wf.prepareParagraph(_flatten(text, family: family))
            .maxIntrinsicWidth;
      }, warmup: 1, runs: corpusRuns);
      final preparedCorpus = wf.prepareParagraph(_flatten(text, family: family));
      final hotLayout = await bench((r) {
        final width = _widths[r % _widths.length];
        sink += wf
            .layoutPreparedLines(
                preparedCorpus, width, wf.ParagraphStyle(maxWidth: width))
            .height;
      }, sampleRepeats: _layoutRepeats, warmup: 1, runs: corpusRuns);
      final lineCount = wf
          .layoutPreparedLines(
              preparedCorpus, 300, const wf.ParagraphStyle(maxWidth: 300))
          .lines
          .length;
      report.cpuResults.add(cpuResult(
        id: 'cpu.corpus_long/$id',
      ...
      final rtPainter =
          TextPainter(text: _span(text, family: family), textDirection: TextDirection.ltr);
      final rtCold = await bench((_) {
      ...
      final rtHot = await bench((r) {
        final width = _widths[r % _widths.length];
        rtPainter.layout(maxWidth: width);
        sink += rtPainter.height;
      }, sampleRepeats: _widths.length, warmup: 1, runs: corpusRuns);
      ...
      report.cpuResults.add(cpuResult(
        ...
        desc: 'chars=${text.length}; two layouts per run (no clearable cold '
            'path in the engine)',
        path: 'pure',
        runMs: rtCold,
        ...
        sink += wf
            .layoutPreparedLines(
                ...
                const wf.ParagraphStyle(
                    maxWidth: 300, align: wf.TextAlign.justify))
            .height;
      }
    }, sampleRepeats: 5, runs: _runs);
    final kp = await bench((_) {
      for (final p in kpParas) {
        sink += wf
            .layoutPreparedLines(
                p,
    ...
    final split = await bench((r) {
      final width = _widths[r % _widths.length];
      sink += wf
          .layoutPreparedLines(
              perfPrepared, width, wf.ParagraphStyle(maxWidth: width))
          .height;
    }, sampleRepeats: 40, runs: _runs);
    final oneshot = await bench((r) {
      final width = _widths[r % _widths.length];
      sink += wf
