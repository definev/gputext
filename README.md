...
Windfoil vector text renderer ported to Flutter GPU. `WindfoilRichText` is a
...
`WINDFOIL_DEMO=<page> flutter run -d macos` opens a page directly:
`widgets`, `pretext`, `justify`, `vars`, or `bench`.

## Benchmarks

A full-matrix RichText vs WindfoilRichText benchmark lives in `lib/bench/`
...
WINDFOIL_BENCH_FILTER=frame.zoom,cpu. tool/run_bench.sh   # subset by id prefix
...
report from stdout (`WFBENCH:` marker lines; the sandboxed app cannot write
...
- **mem.*** — RSS plus windfoil-internal accounting (atlas bytes, cached
...
windfoil; `hybrid`/`cache-disabled` rows exercise the native-delegation and
WidgetSpan paths and are not comparable with pure rows. Windfoil's
